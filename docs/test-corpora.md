...
4. Repository-owned tests cover framing precedence and request-smuggling regressions, byte-at-a-time feeds, every split point, informational/final response association, enforced callback reentrancy guards, callback-panic recovery for lifecycle reset, cap-limited spans, cumulative chunk quotas, sticky failures, upgrades, trailers, truncation, and zero-allocation hot paths. Fuzzing independently exercises arbitrary segmentation, zero-length feeds, randomized and maximal limits, limit monotonicity, callback trace equivalence for committed messages, response-context pipelines, sticky terminal states, reset/init equivalence, lifecycle operation sequences, grammar-generated valid messages, structured framing, chunk, target, Host, and response-line mutations, and full-message fixed/chunked pipeline boundary agreement with Go's independent `net/http` parser on a shared strict subset. `http/parser.go` currently has 100% statement coverage; `scripts/check-http1-coverage.sh` enforces a minimum of 98%.
